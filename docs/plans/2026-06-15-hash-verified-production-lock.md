...
`requirements.txt` and an exact five-package graph in `requirements.lock`.
...
verification a narrow supply-chain hardening step rather than a dependency or
runtime migration.
...
existing Tornado, PyMySQL, cryptography, cffi, and pycparser versions and the
supported Python 3.10, 3.12, and 3.14 matrix.

## Requirements

- Keep the three exact direct pins in `requirements.txt` unchanged.
- Regenerate `requirements.lock` as the same exact five-package graph with
...
versions remain exactly cffi 2.0.0, cryptography 48.0.0, pycparser 3.0,
PyMySQL 1.2.0, and Tornado 6.5.6. Do not upgrade or add runtime packages in
this change.
...
- Parse the generated lock and prove the exact five-package graph, exact
...
- Do not change application code, dependency versions, or public behavior.
