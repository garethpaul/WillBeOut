...
def validate(workflow):
    ...
    if len(re.findall(r"^    runs-on: ubuntu-24\.04$",workflow,re.MULTILINE))!=1: errors.append("use the fixed Ubuntu runner exactly once")
    if len(re.findall(r"^    timeout-minutes: 5$",workflow,re.MULTILINE))!=1: errors.append("bound the job to five minutes exactly once")
    ...
    if CHECKOUT_BLOCK not in workflow: errors.append("use the exact credential-free checkout contract")
    ...
    if re.search(r"\b(?:pip|pip3) install\b",workflow): errors.append("not install dependencies for the static contract")
    return errors
